...
if(workplacejob_hostage_active)exitWith{player groupChat "There are no more hostages right now";};
if(workplacejob_hostage_failed)exitWith{player groupChat "You have failed a hostage mission recently, try again later.";};
...
"if (iscop) then {player sideChat ""Someone has taken a hostage! The hostage has been marked on the map. The hostage taker must be arrested or killed in order for the hostage to go free!""};" call broadcast;
...
while {true} do 
	...
	*/
	...
	if (_minutecounter >= 1200 and alive player) exitWith
		...
		{
		kontostand = kontostand + 400000;
		player groupchat "Well done, you kept the hostage for 20 minutes, $400000 has been transfered to your account.";
		sleep 10;
	...
	if(hostage1 distance getmarkerpos "hostagezone" > 125) exitwith
		...
		"
		server globalchat ""The Hostage has escaped! Cops get $50000"";
