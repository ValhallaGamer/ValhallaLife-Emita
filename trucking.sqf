...
_vcl = (nearestobjects [getpos player, ["cl_del_mackr","cl_trucktest_mackr","cl_flatbed_mackr"], 15] select 0);
//Legal truck mission
if (_art == 1) then 
{
...
if (_newmarker == 1) then {_shopo = "Gas Haus";};
if (_newmarker == 2) then {_shopo = "Lake Placid Food Stand";};
if (_newmarker == 3) then {_shopo = "Muscle Car Shop";};
...
player groupchat format["Once there talk to the %1", _shopo];
	...
	if (player distance _markerlocation <= 15) then 
		{
...
{
_paid = 30000;
_cargo = (nearestobjects [getpos player, ["Land_bags_stack_EP1","Land_bags_EP1","Land_Bag_EP1"], 15] select 0);
...
{deleteVehicle _x;_deleted = _deleted + 1;} forEach (nearestObjects [player,["Land_bags_stack_EP1","Land_Bag_EP1"], 15]);
...
_markername1 Setmarkerposlocal _markerlocation1;
	...
	if (player distance _markerlocation1 <= 15) then 
		{
