...
sleep 0.001;
execVM "disablebuttons.sqf";
if (_this select 0 != player) exitWith {};
ss_Player = _this select 0;
ss_Killer = _this select 1;
_loc = nearestlocations [player,["NameCityCapital","NameCity","NameVillage"],50000] select 0;
_coords = [position player] call fn_Coords;
format["if(playerSide == resistance) then {player sideChat ""%1 is unconscious and requires emergency medical assistance near %2 [%3,%4]""; titleText[""%1 is unconscious and requires emergency medical assistance near %2 [%3,%4]"",""PLAIN DOWN""];  playSound ""ESU_Alert"";};",name player,text _loc,(toString(_coords select 0)),(toString(_coords select 1))] call broadcast;
...
R3F_REV_fil_exec_attente_reanimation = [] spawn
{
	...
	if (R3F_REV_nb_reanimations > 0) then
	{
		...
		(vehicle player) allowDamage false;
		sleep 0.001;
        if (({side _x == resistance} count allUnits) < 2) then 
		{
			deleteMarker ("R3F_REV_mark_" + name player);
			[] spawn R3F_REV_FNCT_reapparaitre_camp;
			player sideChat "No ESU Are online, you have been respawned.";
			_btnAbort ctrlEnable false;
		};
		revivetimer = 1;
		execVM "revivetimer.sqf";
