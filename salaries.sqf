...
if ("patrol_training" call INV_HasLicense) then 
	...
	_income = _income + (2500 + random 1000 - random 500);
...
if ("response_training" call INV_HasLicense) then 
	...
	_income = _income + (3500 + random 1000 - random 500);
...
if ("swat_training" call INV_HasLicense) then 
	...
	_income = _income + (4500 + random 2000 - random 1000);
...
sleep 1;					
if(ischief)then{player groupchat format["As the Police Chief you get an extra paycheck of $%1.", (chiefExtraPay call ISSE_str_IntToStr)]};   
...
*/		
	...
	if (isMayor) then 
		...
		MayorSteuern = round((MayorSteuern / 150) * MayorBekommtSteuern); 									
